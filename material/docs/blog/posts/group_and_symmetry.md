...
- $\exp : (\R, +) \to \R^\times$
    ...
    这里我们用$(\R, +)$表示群$\R$上的乘法定义为实数加法。此外$\R^\times$将$\R$中的$0$排除在外。我们有$\exp(x + y) = \exp(x)\exp(y)$，因此指数映射是一个同态。这个同态不是满射，因此它不是同构。但存在一个从$(\R, +)$到$\R^+$的同构$\exp : (\R, +) \to \R^+$，其逆称为对数映射。
...
- $f: \Z/4\Z \to (\Z/5\Z)^\times$

    $f(a) = 2^a (\mathrm{mod} 5)$

    这可以看作指数映射在有限域上的情形，现在$f$是一个同构。
    
    注：$\Z/4\Z \simeq (\Z/5\Z)^\times$中的元素是$\Z/5\Z$的自同构。
...
费马定理是说$x^p \equiv x \mod p$. 从拉格朗日定理出发，既然$G$中元素$g$的阶整除$|G|$，我们就有$g^{|G|} = 1$. 考虑$G = (\Z / p\Z)^\times$，其阶为$p - 1$；所以当$x \ne 0$时我们有$x^{p - 1} \equiv 1 \mod p$。于是$x^p \equiv x \mod p$ 即使在$x = 0$时也是成立的。
...
为证明欧拉定理，我们考虑$G = (\Z/m\Z)^\times$，即$\mod m$意义下所有与$m$互素的整数，可以验证它们构成一个群。于是$x^{|G|} = x^{\varphi(m)} \equiv 1$，其中$x \in (\Z/m\Z)^\times$，显然与$m$互素。

我们可以验证一下，例如$(\Z/12\Z)^\times = \{1, 5, 7, 11\}$，$\varphi(12) = 4$. 我们有$1^4 \equiv 1, 5^4 \equiv 1, 7^4 \equiv 1, 11^4 \equiv 1$. 实际上，$1^2 \equiv 1, 5^2 \equiv 1, 7^2 \equiv 1, 11^2 \equiv 1$. 所以欧拉的结果实际上并不是最好的。
...
将群运算取为$+$，则$G$可以看作是有限域（$p$个元素）$\mathbb F_p = \Z/p\Z$上的矢量空间，满足$px = 0$。

作为矢量空间，我们有$G \simeq (\mathbb F_p)^n$，而$n$维矢量空间在同构的意义下是唯一的，于是这样的$G$作为群也是唯一的，其阶为$p^n$. 它们被称为初等阿贝尔$p$-群，某种意义上是除循环群之外最简单的一类群了。
...
承载此群的集合就是$G$和$H$的笛卡尔积。群运算是显然的，$(g_1, h_1)(g_2, h_2) = (g_1g_2, h_1h_2)$.
...
比如，考虑交换子$[a, b] := a^{-1}b^{-1}ab$。在$A, B$都是正规子群的情形下，我们有$[a, b] \in A \land [a, b] \in B$. 于是$[a, b] = 1$.
反之，若$A, B$对易，我们就有$gAg^{-1} = abAb^{-1}a^{-1} = A$. 于是$A, B$皆为正规子群。

下面给出直积的一些例子。

- $\R^\times \simeq \{1, -1\} \times \R^+$
- $\mathbb C^\times \simeq \mathrm S^1 \times \R^+$  
...
- $(\Z/mn\Z)^\times \simeq (\Z/m\Z)^\times \times (\Z/n\Z)^\times$
    ...
    这里$(\Z/m\Z)^\times$的意思是我们仅取与$m$互素的那些元素，并考虑它们在乘法下形成的群。

    具体的一个例子是$(\Z/15\Z)^\times \simeq (\Z/5\Z)^\times \times (\Z/3\Z)^\times$. 其中$(\Z/15\Z)^\times$的阶为8. 而其余两个群我们在之前已经遇到过，它们的阶分别为4和2.
    由欧拉定理，我们得到$x^8 \equiv 1 \mod 15$. 然而，由于$(\Z/15\Z)^\times \simeq \Z/4\Z \times \Z/2\Z$这个分解，我们知道$\Z/4\Z \times \Z/2\Z$这个群的元素的阶必整除4.
    所以实际上我们有$x^4 \equiv 1 \mod 15$. 于是使用像这样的群分解我们可以改进欧拉定理的结果。

以下是一些不那么显然的例子。

- $\mathbb Q^\times \simeq \Z/2\Z \times (\Z \oplus \Z \oplus \Z \oplus \cdots)$
...
这意味着$G$应该有一个到$G/H$的满同态，其核恰好为（或者说同构于）$H$. 这里的一种可能性是令$G/H$为所有$H$的左陪集的集合。这样$G$就通过群运算作用于$G/H$. 前面我们已经知道了这样的作用是传递的。现在的问题是，我们能将这个集合提升为一个群吗？比如说，我们可能想把群运算定义为$g_1H \cdot g_2H = g_1g_2H$. 问题在于，有可能存在一个$g_3 = g_1h \in g_1H$，满足$g_3H = g_1H$，这时我们有$g_3g_2H = g_1g_2H$吗？注意这意味着$g_1hg_2H = g_1g_2(g_2^{-1}hg_2)H = g_1g_2H$. 这就是说，如果$\forall h \in H: g_2^{-1}hg_2 \in H$，则上述条件就是成立的，我们的定义就是合理的。实际上，容易验证以下的条件皆是互相等价的：

- $H$是某个映射$G \to X$的核
- $\forall g \in G: gHg^{-1} = H$
- $\forall g \in G: gH = Hg$
- $H$的左陪集和右陪集相同
- 商群$G/H$存在，其群运算正如上述定义
...
显然，我们可以取$G$为集合$A\times B$，并定义群运算为$(a_1, b_1)(a_2, b_2) = (a_1a_2, b_1^{a_2}b_2)$，因为若我们希望$A$是$G$的子群且$B$是$G$的正规子群，我们只能这么定义。问题是，这样定义确实令$G$成为一个群了吗？
读者可以检查这样定义的群的单位元为$(1, 1)$，逆元为$(a^{-1}, (b^{-1})^{a^{-1}})$，因此只需检查该群运算满足结合律。
...
- $ax + b$群
    ...
    它有一个正规子群$\R$，即所有形如$x \mapsto x + b$这样的变换；以及一个子群$\R^\times$，即所有$x \mapsto ax$这样的变换，其中$a \ne 0$.
    这两个子群互不对易。我们定义$\R^\times$于$\R$上的左作用为$b^a = aba^{-1} = ab$.
...
我们知道单位复数$|z| = 1$构成一个圆环$S^1$，它实际上是一个群，群运算就是复数乘法。
...
$\Z/p\Z$的自同构群为$(\Z/p\Z)^\times$，它只有两个2阶元。这是因为$\Z/p\Z$实际上形成一个有限域，而在（有限）域中$n$次多项式至多有$n$个根。
考虑方程
...
<aside>正如二元多面体群是多面体群在二重覆叠下的原像，二元循环群定义为循环群在二重覆叠下的原像</aside>
...
\begin{document}
\begin{tikzcd}
                                                                                   &                                          & \mathbb Z/3\mathbb Z \rtimes \mathbb Z/4\mathbb Z \arrow[ld, no head] \arrow[rd, no head] &                                          \\
...
13，14阶群比较简单，来看15阶群。更一般地，我们考虑阶为$pq$的群$G$，其中$p, q$皆为素数。不失一般性地，我们设$p < q$. 根据西罗定理，$G$有阶为$p, q$的子群。其中$q$阶子群的数量要整除$p$，且等于$1 \mod q$，所以只能是1，故为正规子群。
所以$G$是一个半直积$\Z/q\Z\rtimes\Z/p\Z$. 问题是，$\Z/p\Z$于$\Z/q\Z$之上可以有什么样的作用？如果我们能搞清楚这点，我们就搞清楚了这个群的结构。形式地说，这个作用就是$\Z/p\Z$到$\Z/q\Z$的自同构的同态，$\Z/p\Z \to \mathrm{Aut}(\Z/q\Z)$. 于是我们要问，$\Z/q\Z$的自同构有哪些？更一般地，我们可以问$\Z/n\Z$的自同构有哪些，其中$n$可以是任意正整数。我们可以先考虑同态，实际上每个映射$1 \mapsto g$都给出一个同态，因为后续只要把每个$n \mapsto ng$即可。可以证明，若$g$在$(\Z/n\Z)^\times$中是可逆的，则这个同态就成为同构。其中$(\Z/n\Z)^\times$是所有与$n$互素的元素构成的乘法群，这些元素同时也是环$\Z/n\Z$的单位（也叫可逆元）。实际上，任何一个阿贝尔群的自同态总是构成一个环，其中乘法定义为同态复合，加法定义为同态相加。而且这个自同态环恰好同构于作为环的这个阿贝尔群，在这个同构下，$r$被映射为把每个元素自加$r$次的那个自同态。当且仅当$r, n$互素时，这个自同态（每个元素自加$r$次）是一个双射。于是$\Z/n\Z$的自同构群就是单位群$(\Z/n\Z)^\times$.

很容易对前几个结果进行计算，如下表（其中用$\bar{2}$这样的标识标出了生成元）

| $n$ | $\varphi(n)$ | $(\Z/n\Z)^\times$ |
| -- | -- |-- |
| 1 | 1 | $1$ |
| 2 | 1 | $1$ |
| 3 | 2 | $1, \bar 2$ |
...
| 5 | 4 | $1, \bar 2, \bar 3, 4$ |
| 6 | 2 | $1, \bar 5$ |
| 7 | 6 | $1, 2, \bar 3, 4, \bar 5, 6$ |
| 8 | 4 | $1, 3, 5, 7$ （非循环群） |
| 9 | 6 | $1, \bar 2, 4, \bar 5, 7, 8$ |
| 10 | 4 | $1, \bar 3, \bar 7, 9$ |
| 11 | 10 | $1, \bar 2, 3, 4, 5, \bar 6, \bar 7, \bar 8, 9, 10$ |
| 12 | 4 | $1, 5, 7, 11$ （非循环群） |

$(\Z/n\Z)^\times$都是阿贝尔群，这里要证明的一个主要的结果是，若$n = p$是素数，则$(\Z/p\Z)^\times$是循环群。

当$n = p$是素数时，$\Z/p\Z$构成一个域；在域中方程$x^d = 1$至多有$d$个根，故$(\Z/p\Z)^\times$中至多有$d$个元素满足$x^d = 1$. 实际上，$(\Z/p\Z)^\times$中至多有$\varphi(d)$个阶为$d$的元素。欧拉$\varphi$函数有如下的性质

$$\sum_{d|m}\varphi(d) = m$$

只要考虑$\Z/m\Z$，并对其中阶为$d$的元素分别计数就可看出这一点。但任何满足$d$阶元至多有$\varphi(d)$个的群$G$必须恰有$\varphi(d)$个$d$阶元，从而一定是循环群。因为从

$$\sum_{d||G|}\varphi(d) = |G|$$

我们可以看出，如若不然将造成某个$d'$阶元的个数大于$\varphi(d')$. 该循环群的生成元有时也被称为原根，此术语可以追溯到数论中。

现在回到阶为$pq$的群$G = \Z/q\Z\rtimes\Z/p\Z$的分类，我们看到$\Z/q\Z$的自同构是$\Z/(q-1)\Z$，然而$\Z/p\Z$到$\Z/(q-1)\Z$没有非平凡的同态映射，除非$p$整除$q - 1$.
这是因为非平凡同态保持群元的阶，$\Z/(q-1)\Z$的元素的阶都整除$q - 1$，这就要求$\Z/p\Z$的元素的阶也都整除$q - 1$.
如果$p$确实整除$q - 1$，我们就得到半直积$G = \Z/q\Z\rtimes\Z/p\Z$.
总之，对于阶为$pq$（其中$p < q$）的群$G$来说，若$p$与$q - 1$互素，则我们得到唯一的直积；否则，我们得到一个直积和一个半直积。

注意当$p = 2$时，我们总有$2$整除$q - 1$，这种情况下的半直积给出的正是二面体群$D_{2q}$.

当$n = p_1^{a_1}p_2^{a_2}\cdots$不是素数时，我们有

$$(\Z/n\Z)^\times \simeq (\Z/p_1^{a_1}\Z)^\times \times (\Z/p_2^{a_2}\Z)^\times \cdots$$

所以我们可以假定$n$为素数幂，此时若$p \ne 2$我们有$(\Z/p^n\Z)^\times$为循环群，其阶为$\varphi(p^n) = p^{n-1}(p - 1)$.

$$(\Z/p^n\Z)^\times \simeq \Z/p^{n-1}\Z\times\Z/(p-1)\Z$$

其中$p + 1 \in (\Z/p^n\Z)^\times$是$\Z/p^{n-1}\Z$的生成元。若$p = 2$，我们有

$$(\Z/2^n\Z)^\times \simeq \Z/2\Z\times\Z/2^{n-2}\Z$$

其中$5 \in (\Z/2^n\Z)^\times$是$\Z/2^{n-2}\Z$的生成元。

> 为什么$p = 2$时会出问题？实际上，我们希望$1 + p \in (\Z/p^n\Z)^\times$的阶为$p^{n-1}$对所有的$p$都成立，这要求$(1 + p)^{p^{n-2}} \not\equiv 1 \mod p^n$.
> 使用二项式展开我们看到$(1 + p)^{p^{n-2}} = 1 + p^{n-2}p + \binom{p^{n-2}}{2}p^2 + \binom{p^{n-2}}{3}p^3 + \cdots$
> 其中$\binom{p^{n-2}}{3}p^3$及以后各项都可被$p^n$整除，$\binom{p^{n-2}}{2}p^2$通常也可被$p^n$整除，故整个式子通常$\not\equiv 1 \mod p$.
> 但$p = 2$时，$\binom{p^{n-2}}{2}p^2$就不可被$p^n$整除了，实际上这时上式的结果为$1 + p^{2n - 3} \mod p^n$，当$n > 2$时就是$1 \mod p^n$了。
> 就是因为这个二项式系数里分母中的$2$引起了这个问题。

我们以上述结论的一个实例结束本节。假设我们想知道$(\Z/1000000\Z)^\times$的结构，我们可以将其分解为$(\Z/2^6\Z)^\times\times(\Z/5^6\Z)^\times$，应用上述结论$(\Z/2^6\Z)^\times \simeq \Z/2\Z\times \Z/2^4\Z$，而$(\Z/5^6\Z)^\times \simeq \Z/5^5\Z\times \Z/4\Z$，我们最终得到

$$(\Z/1000000\Z)^\times \simeq \Z/2\Z\times \Z/2^4\Z\times\Z/5^5\Z\times \Z/2^2\Z$$

## 有限阿贝尔群

现在来到阶为16的群，它的个数有点多，所以本节我们先来考虑阶为16的阿贝尔群。
比如$\Z/16\Z, \Z/8\Z\times\Z/2\Z, \Z/4\Z\times\Z/4\Z, \Z/4\Z\times\Z/2\Z\times\Z/2\Z, \Z/2\Z\times\Z/2\Z\times\Z/2\Z\times\Z/2\Z$这些都显然是阶为16的阿贝尔群。
问题是，还有其他的吗？如果我们回忆以下直到现在我们遇到过的阿贝尔群，它们都可以写成循环群的积。例如上一节中我们看到$(\Z/n\Z)^\times$就都可以被写成循环群的积。
很明显地，并非任何阿贝尔群都可以写成循环群的积，例如有理数集$\mathbb Q$在以加法为群运算时是阿贝尔群，但它不可表为循环群的积。
不过，下面我们要说明，每个有限（生成的）阿贝尔群确实可以写成循环群$\Z/p^n\Z$（及$\Z$）的积。
实际上，可以证明更一般的结论，即每个欧几里得环$R$上的有限生成模都可以写成循环模$R/aR$的和。
这里如果我们取$R$为整数环，则这个环上的有限生成模就恰好是阿贝尔群。
我们要用到的是整数的如下性质：给定整数$a, b$，其中$b \ne 0$，存在$q, r$，使得$a = qb + r$且$|r| < |b|$.
其中$q$称为$a$除以$b$的商，$r$则称为余数。

考虑阿贝尔群$G$（以加法为群运算）的一些生成元$g_1, \dots, g_k$，称施加于它们之上得$0$的群操作为它们之间的关系，例如

$$n_{11}g_1 + n_{12}g_2 + \cdots + n_{1k}g_k = 0$$

其中$n_{ij} \in \Z$. 当然，我们可以有多个关系，例如我们还有

$$\begin{align*}
n_{21}g_1 + n_{22}g_2 + &\cdots + n_{2k}g_k = 0 \\
n_{31}g_1 + n_{32}g_2 + &\cdots + n_{3k}g_k = 0 \\
&\dots
\end{align*}$$

关系的个数甚至可以是无穷的。这里的关键点是，我们要写下足够多的关系来确保这些生成元的确生成我们想要的群。
然后我们会操作这些关系（而不改变我们想要的群）让它们有一个较好的形式。简洁起见，我们可以把所有这些$n_{ij}$写成一个（可能是无限的）矩阵。
这些矩阵上的哪些操作不改变它所决定的群呢？我们可以：

- 将某一列的倍数加到另一列上，这等价于将生成元$g, h, \dots$替换为$g, h+ng, \dots$
- 将某一行的倍数加到另一行上，因为关系$R_1, R_2, \dots$和关系$R_1, R_2+nR_1, \dots$是等价的
- 交换任意两行或交换任意两列，因为这不改变这些关系

接下来，我们就进行这样的操作，使得$|n_{11}|$尽可能的小但不为零。
然后，继续操作使得第一行和第一列的其他元素都变为零。
之所以可以这样做是因为这些元素都是$n_{11}$的倍数；
如若不然，我们就令这些元素减去$n_{11}$的某个倍数，根据前面提到的整数的性质，我们将得到一个小于$|n_{11}|$的余数，然后交换这两者我们就可以得到一个更小的$n_{11}$.

然后我们就对$n_{22}, n_{33}, \dots$重复以上操作，并扔掉全为零的行，不难看出最终我们将得到如下的关系

$$n_{11}g_1 = 0, n_{22}g_2 = 0, \dots, n_{kk}g_k = 0$$

于是群$G$就同构于$\Z/n_{11}\Z\times\Z/n_{22}\Z\times\cdots\times\Z/n_{kk}\Z$，其中$\Z/n_{11}\Z$由$g_1$生成，$\Z/n_{22}\Z$由$g_2$生成等等。
也可以按素因子分解$n = p_1^{n_1}p_2^{n_2}\cdots$将$\Z/n\Z$写成$\Z/p_1^{n_1}\Z\times\Z/p_2^{n_2}\Z\times\cdots$.
当$n = 0$时，我们有$\Z/0\Z \simeq \Z$.

这个结果当然很好，但这可能也意味着有限（生成）的阿贝尔群因其简单的已知的结构，不会有什么有意思的性质了。

我们当然可以问，什么时候两个有限阿贝尔群是同构的？换言之，设

$$G \simeq \Z/q_1\Z\times\Z/q_2\Z\times\cdots$$

其中$q_1, q_2, \dots$是（无需不同的）素数的幂，是否意味着$q_1, q_2, \dots$在忽略其顺序的前提下被唯一确定了呢？注意如果$q_1, q_2, \dots$不是素数的幂，答案将是否定的，例如$\Z/6\Z \simeq \Z/2\Z\times\Z/3\Z$. 但在$q_1, q_2, \dots$确为素数幂的情况下，答案是肯定的。注意将$n$分解为$a + b + c + \cdots$（其中$a \ge b \ge c \ge \cdots > 0$）其分解方式的个数和$p^n$阶阿贝尔群$\Z/p^a\Z\times\Z/p^b\Z\times\Z/p^c\Z\times\cdots$的个数之间有一一对应的关系，所以$p^n$阶阿贝尔群的个数就等于$n$的分拆数$p(n)$，$p$称为分拆函数。定义$p(0) = 1$，其从$0$开始的几个函数值为$1, 1, 2, 3, 5, 7, \dots$

最后需要说明的是，结论“每个有限群都是形如$\Z/p^n\Z$这样的循环群的积”和结论“每个复矩阵都有若尔当标准型”有密切的关系。前者是关于整数环$\Z$上的模的结论，后者是关于复多项式环$\mathbb C[x]$上的模的结论。

## 幂零群

前面我们已经知道，所有有限$p$-群都是幂零的，16阶群自然也不例外。
容易证明，两个幂零群的积也是幂零的。于是任意有限$p$-群的积也都是幂零的。
反过来，我们将要说明，任何有限幂零群也都是$p$-群的积。实际上，以下命题都是等价的

- 有限群$G$是$p$-群的积
- 有限群$G$是幂零的
- 有限群$G$所有的西罗子群都是正规的（各阶西罗$p$-子群仅一个）

首先，假设对于阶小于$G$的幂零群的所有西罗子群都是正规的。由于$p$-群皆有非平凡的中心，且其阶为$p$的倍数，故根据柯西定理，我们可取$g \in Z(G)$使得$g^p = 1$.
考虑$G/\braket g$，容易看出它仍是幂零的，根据我们的假设，其西罗$q$-子群就是正规的。
若$q = p$，则该西罗$q$-子群在$G$中的原像仍为正规的西罗$q$-子群。
若$q \ne p$，则该西罗$q$-子群在$G$中的原像的阶为$pq^n$，其中$q^n$是能整除$|G|$的最大幂。
注意这个原像有一个阶至少为$p$的中心（因为$\braket g$就是由中心里的$p$阶元生成的），且它有一个阶为$q^n$的西罗$q$-子群。
这个西罗子群必为正规的，因为唯一的共轭类是由来自中心的共轭作用给出的，而该共轭作用保持这个西罗子群不变。

然后我们说明若群$G$所有的西罗子群都是正规的，则该群是这些西罗子群的积。
取西罗$p$-子群$P$和西罗$q$-子群$Q$，且设$p \ne q$；因为$P, Q$是正规的，故交换子$aba^{-1}b^{-1} \in P \cap Q = 1$，其中$a \in P, b \in Q$.
这就是说群$G$的任何西罗$p$-子群和西罗$q$-子群都是对易的，于是群$G$就是这些西罗子群的积。

至此，我们就完成了上述三个命题等价的证明。

最后，16阶群中的非阿贝尔群是

- $\Z/2\Z\times D_8$
- $\Z/2\Z\times Q_8$ 三个2阶元，十二个4阶元
- 广义四元数群（二元二面体群）
- $\Z/8\Z\rtimes\Z/2\Z$
    - $bab^{-1} = a^3$ 拟二面体群
    - $bab^{-1} = a^5$ 模群
    - $bab^{-1} = a^7$ 二面体群
- $\Z/4\Z\rtimes\Z/4\Z$ 三个2阶元，十二个4阶元
- $(\Z/2\Z\times\Z/2\Z)\rtimes\Z/4\Z$ 七个2阶元，八个4阶元
- $(\Z/4\Z\times\Z/2\Z)\rtimes\Z/2\Z$ 泡利群 七个2阶元，八个4阶元

实际上，对每个$n \ge 4$，阶为$2^n$且有指标为2的循环子群的非阿贝尔群恰好是四个，它们是广义四元数群，二面体群，拟/半二面体群（$bab^{-1} = a^{2^{n-2} - 1}$）和模群（$bab^{-1} = a^{2^{n-2} + 1}$）。

## 圈积

现在来看阶为18的群$G$，根据西罗定理，它有9阶的西罗子群，而且是正规的。所以$G$必为该9阶群与$\Z/2\Z$的半直积。
我们知道9阶群要么是$\Z/9\Z$，要么是$\Z/3\Z\times\Z/3\Z$. 在$\Z/9\Z$的情况我们得到$\Z/18\Z$或者是$D_{18}$.
考虑$\Z/3\Z\times\Z/3\Z$的情况，我们可以将其视为$\mathbb F_3$上的二维矢量空间$V$，而$\Z/2\Z$中的2阶元对应于其上的2阶线性变换$\sigma$，我们有$\sigma^2 = 1$.
$\sigma$可以将坐标对调，也可以将两个坐标都变为其逆，也可以都不变；类似于矩阵$\begin{pmatrix}0 &1 \\ 1 &0\end{pmatrix}, \begin{pmatrix}-1 &0 \\ 0 &-1\end{pmatrix}$和$\begin{pmatrix}1 &0 \\ 0 &1\end{pmatrix}$.
第一种对应于$\Z/3\Z\times S_3$，这是一种称为圈积的构造；第二种对应于$(\Z/3\Z\times\Z/3\Z)\rtimes\Z/2\Z$，其中$\Z/2\Z$以取逆的方式作用于$(\Z/3\Z\times\Z/3\Z)$，称为弗罗贝尼乌斯群，我们下节讨论；最后一种对应于$(\Z/3\Z\times\Z/3\Z)\times\Z/2\Z$.

给定两个群$G, H$，其圈积$G\wr H$定义为$H$个$G$的直积与$H$的半直积，即

$$(G\times G \times \cdots \times G)\rtimes H$$

这里的例子$\Z/3\Z\wr\Z/2\Z$，$\Z/2\Z$是以坐标对调的方式作用于$(\Z/3\Z\times\Z/3\Z)$. 我们之前遇到过的$D_8$也是圈积的例子，它相当于$\Z/2\Z\wr\Z/2\Z$.
圈积可以看成是（图论/数据结构中的）树的自同构群。
此外，圈积还与对称群的西罗子群有关。
例如$S_{27}$，我们可以一棵27个叶节点的完全3叉树表示之。

$$27! = 2^{23}\times3^{13}\times5^6\times7^3\times11^2\times13^2\times17\times19\times23$$

因此$S_{27}$的西罗$3$-子群为$3^{13}$阶。考虑同属一个父节点的三个叶节点的轮换，如$(123), (456), (789)$等。它们中的每一个对应于$\Z/3\Z$，那么1至9这九个元素的轮换就对应于西罗$3$-子群$(\Z/3\Z)^3$.
再考虑它们所在的三个父节点的轮换，且我们要求父节点的轮换不影响子节点的顺序，这又给出一组轮换。整体来看，这相当于$\Z/3\Z\wr\Z/3\Z$.
然而，这三个父节点又属于更高一层的父节点，这样的父节点也有三个，于是最终我们得到的是

$$(\Z/3\Z\wr\Z/3\Z)\wr\Z/3\Z$$

容易计算这个群的阶正好是$3^{13}$. 实际上，$3$可以替换成任何其他的素数，我们都将得到这样的圈积的塔。
如果$S_n$中的$n$恰好是$p$的幂，则其西罗$p$-子群将对应一棵完整的树。一般情况下，西罗子群将是形如这样的圈积的塔的直积。
例如对于$S_{13}$，其西罗$2$-子群为$2^{10}$阶；因$13 = 2^3 + 2^2 + 2^0$，我们得到三棵完全二叉树，其高度分别为$3, 2, 0$.
从中我们即可得到$\Z/2\Z\wr\Z/2\Z\wr\Z/2\Z\times\Z/2\Z\wr\Z/2\Z\times1$，容易验证其阶数与西罗$2$-子群一致。
对于任一对称群的任一西罗子群，结论将是类似的。反过来，任意$p$-群，因其必包含于某个对称群，因此也必可表为这样的圈积塔的直积的形式。
但这样的圈积塔极其复杂，目前还未能通过枚举这些塔的直积来完成对所有$p$-群的分类。

圈积也出现在物理中，但这里要用到一个更一般化的圈积的定义。定义$G$的直积$G^S$为所有从$S$到$G$的函数的集合。设$H$左作用于集合$S$上，则这个作用可以扩展到$G^S$上

$$hf := f\circ h^{-1}$$

其中$f \in G^S, h \in H$，容易验证这是$H$于$G^S$上的一个左作用。当$G$和$S$上有拓扑结构时，我们进一步要求$f$是连续函数。然后我们定义圈积$G \wr_S H$为$G^S \rtimes H$，当取$S = H$时该定义就回到前述情形。
现在取$S$为时空，取$H$为时空的对称群（庞加莱群或洛伦兹群），取$G$为某种规范群（例如电动力学里我们取$G$为圆群$\mathrm S^1$），所以$G^S$就是所有从时空到$\mathrm S^1$的连续/光滑函数（有时也径直称为规范群），而$G^S\rtimes H$将是量子电动力学中的对称群，有时就直接记为$G\wr H$.
所以在量子场论中，圈积也常常露面。更一般地，当考虑流形$S$上的纤维丛$G$时，$G^S$将被视为纤维丛的截面。

## 弗罗贝尼乌斯群

阶为20的群$G$有西罗$5$-子群，而且是正规子群。所以$G$是$\Z/5\Z$与4阶群的半直积。若这个4阶群为$\Z/2\Z\times \Z/2\Z$，平凡作用的情形下我们得到$\Z/10\Z\times\Z/2\Z$，非平凡作用的情形下我们得到$D_{10}\times\Z/2\Z$.
若4阶群为$\Z/4\Z$，则它有三种方式可以作用于$\mathrm{Aut}(\Z/5\Z) = \Z/4\Z$，平凡作用的情形下我们得到$\Z/20\Z$，非平凡作用有2阶自同构$\Z/5\Z \rtimes \Z/4\Z, bab^{-1} = a^{-1}$和4阶自同构$\Z/5\Z \rtimes_2 \Z/4\Z, bab^{-1} = a^2$.
2阶自同构半直积对应二元二面体群，4阶自同构半直积对应所谓的$ax + b$群，其中$a \in (\Z/5\Z)^\times, b \in \Z/5\Z$.
它同时也是弗罗贝尼乌斯群的一个例子。

所谓弗罗贝尼乌斯群$G$是一个传递地作用于集合$S$上的群，满足

- 其非单位元的稳定点的个数不超过1
- 存在稳定1个点的非单位元

例如，任意域上的$ax + b$群是弗罗贝尼乌斯群，这里$S$是域的元素的集合，群$G$中的元素就是所有形如$ax + b$的变换。
其中$a \in S^\times, b \in S$. 如果$S$对应的域是有限的，则这个群的阶为$|S|(|S| - 1)$.
$|S| = 3$时它对应$S_3$，$|S| = 4$时它对应$A_4$，$|S| = 5$时它对应我们上边提到的群。

此外，$D_{2n}$当$n$为奇数时也是弗罗贝尼乌斯群，其中$S$是多边形的各顶点的集合。
容易看到，旋转不存在稳定点；但奇多边形的反射稳定1个点，而偶多边形存在可稳定2个点的反射。
更一般地，如果$K$是任意奇数阶的阿贝尔群，而2阶的$H$以取逆的方式作用于$K$，则$K \rtimes H$为弗罗贝尼乌斯群。
于是上节中的$(\Z/3\Z\times\Z/3\Z)\rtimes\Z/2\Z$也是弗罗贝尼乌斯群。

关于弗罗贝尼乌斯群有两个重要的结论，第一个来自弗罗贝尼乌斯：设$H$为$G$的稳定1个点的子群，$K$为不在$H$共轭类中（不稳定任何点的）元素和单位元之集合；弗罗贝尼乌斯断言$K$是一个正规子群，且$G \simeq K \rtimes H$.
这里$K$称为弗罗贝尼乌斯核，弗罗贝尼乌斯证明存在一个群同态$G \to H$，其核为$K$.
第二个来自汤普森，断言弗罗贝尼乌斯核$K$是幂零的。如果考察我们目前遇到的弗罗贝尼乌斯群，读者会发现它们的$K$不仅是幂零的，而且是阿贝尔的。
$K$为幂零但非阿贝尔的例子有点难找，其中一个如下：令$K = \begin{pmatrix}1&*&* \\ 0&1&* \\ 0&0&1\end{pmatrix} \in \mathrm{GL}_3(\mathbb F_7), H = \left<\begin{pmatrix}1&0&0 \\ 0&2&0 \\ 0&0&4\end{pmatrix}\right>$，则$K\rtimes H$是弗罗贝尼乌斯群，其阶为$7^3\times 3$.

阶为21的群符合$pq$的情况，其中$p = 3, q = 7$. 于是它有两种可能性$\Z/21\Z, \Z/7\Z \rtimes \Z/3\Z$，后者也是弗罗贝尼乌斯群。
阶为22的群符合$2p$的情况，于是它可能是$\Z/22\Z$或者$D_{22}$.

## 阶为24的群

阶为24的群$G$有十五个。为此我们考虑其西罗$3$-子群，它可以是正规的（1个），也可以是非正规的（4个）。

如果它是正规的，则$G$是$\Z/3\Z$和一个8阶群的半直积，如果这个8阶群于$\Z/3\Z$的作用是平凡的，我们得到$\Z/3\Z \times \Z/8\Z, \Z/3\Z \times \Z/4\Z \times \Z/2\Z, \Z/3\Z \times (\Z/2\Z)^3, \Z/3\Z\times Q_8, \Z/3\Z \times D_8$;
如果群作用是非平凡的，我们得到$\Z/3\Z \rtimes \Z/8\Z, \Z/3\Z \rtimes (\Z/2\Z)^3, \Z/3\Z\rtimes Q_8$（二元二面体群），以及$\Z/3\Z$和$\Z/4\Z\times \Z/2\Z$的两个半直积，$\Z/3\Z$和$D_8$的两个半直积。
两个半直积间的区别在于其核为$\Z/4\Z$还是$\Z/2\Z\times\Z/2\Z$.
...
| 14 | $C_{14}, D_{14}$ |
| 15 | $C_{15}$ |
| 16 | $C_{16}, C_8\times C_2, C_4\times C_4, C_4\times C_2\times C_2, C_2\times C_2\times C_2\times C_2 \\ C_2\times D_8, C_2\times Q_8, Q_{16}, D_{16}, C_4\rtimes C_4, (C_2\times C_2)\rtimes C_4, (C_4\times C_2)\rtimes C_2, C_8\rtimes_3C_2, C_8\rtimes_5C_2$ |
| 17 | $C_{17}$ |
| 18 | $C_{18}, D_{18}, C_3\times S_3, C_3\times C_6, C_3\wr C_2$ |
| 19 | $C_{19}$ |
| 20 | $C_{20}, C_{10}\times C_2, D_{10}\times C_2, C_5 \rtimes C_4, C_5 \rtimes_2 C_4$ |
| 21 | $C_{21}, C_7 \rtimes C_3$ |
| 22 | $C_{22}, D_{22}$ |
| 23 | $C_{23}$ |
