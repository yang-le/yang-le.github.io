...
<aside>正如二重多面体群是多面体群在二重覆叠下的原像，二元循环群定义为循环群在二重覆叠下的原像</aside>
...
$(\Z/n\Z)^\times$都是阿贝尔群，这里要讨论的一个主要的结果是，若$n$是素数，则$(\Z/n\Z)^\times$是循环群，其阶为$n - 1$.

当$n = p$是素数时，$\Z/p\Z$构成一个域；在域中方程$x^n = 1$至多有$n$个根，故$(\Z/p\Z)^\times$中至多有$n$个元素满足$x^n = 1$. 实际上，$(\Z/p\Z)^\times$中至多有$\varphi(n)$个阶为$n$的元素。欧拉$\varphi$函数有如下的性质
$$\sum_{d|n}\varphi(d) = n$$
只要考虑$\Z/n\Z$，并对其中阶为$d$的元素分别计数就可看出这一点。但任何满足$n$阶元至多有$\varphi(n)$个的群$G$必须恰有$\varphi(n)$个$n$阶元，从而一定是循环群。因为从
$$\sum_{n||G|}\varphi(n) = |G|$$
我们可以看出，如若不然将造成某个$m$阶元的个数大于$\varphi(m)$. 该循环群的生成元有时也被称为原根，该术语可以追溯到数论中。
