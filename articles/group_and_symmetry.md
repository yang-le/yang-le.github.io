...
$\sigma$可以将其中一维变为其逆，也可以将两个维度都变为其逆，也可以都不变；类似于矩阵$\begin{pmatrix}1 &0 \\ 0 &-1\end{pmatrix}, \begin{pmatrix}-1 &0 \\ 0 &-1\end{pmatrix}$和$\begin{pmatrix}1 &0 \\ 0 &1\end{pmatrix}$.
第一种对应于$\Z/3\Z\times S_3$，称为弗罗贝尼乌斯群，我们下节讨论；第二种对应于$(\Z/3\Z\times\Z/3\Z)\rtimes\Z/2\Z$，这是一种称为圈积的构造；最后一种对应于$(\Z/3\Z\times\Z/3\Z)\times\Z/2\Z$.

给定两个群$G, H$，其圈积$G\wr H$定义为$H$个$G$的直积与$H$的半直积，即
$$(G\times G \times \cdots \times G)\rtimes H$$

除了这里的例子$\Z/3\Z\wr\Z/2\Z$外，我们之前遇到过的$D_8$也是圈积的例子，它相当于$\Z/2\Z\wr\Z/2\Z$.
